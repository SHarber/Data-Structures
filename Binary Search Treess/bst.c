...
{
	struct Node *nodeToAdd; //Create pointer to the Node to add.

	//Check to make sure that cur is not null
	if (cur == NULL) //If cur is Null
	{
		nodeToAdd = (struct Node *) malloc(sizeof(struct Node)); //Create a new Node and assign it to node to add
		assert(nodeToAdd != 0); //Assert that node to Add is not 0.
		nodeToAdd->val = val; //Assign the value to the node to Add's value.
		nodeToAdd->left = NULL; //Left node of the new node is null.
		nodeToAdd->right = NULL; //Right node of the new node is null.
		return nodeToAdd; //Return the node to be added.
	}
	//Otherwise compare the value with the right and left nodes of cur
	if (compare(val, cur->val) == -1) //If value is less than the current value
		cur->left = _addNode(cur->left, val); //Recursively call addNode to the left.

	else if (compare(val, cur->val) == 1) //If value is  greater than the current value
		cur->right = _addNode(cur->right, val); //Recursively call addNodeto the right
...
{
	struct Node *current; //Create a Node to hold the current node
	current = tree->root; //Initialize current to theh root of the treee.

	while (current != 0) //Create a loop while current is not (Reached the end of the tree)
	{
		if (compare(val, current->val) == 0) //Compare to see if the val looking for is equal to current's value
			return 1; //If so value is found, and return 1.
		if (compare(val, current->val), -1) //Otherwise see if val is less than current's value.
			current = current->left; //If so, make current equal to current's left.
		else current = current->right; //Otherwise if val is greater, than current val, make current current's right.
	}
	
	return 0; //If value wasn't found return 0.
}
...
{
	while (cur->left != 0) //While curren'ts left is not = 0
		cur = cur->left; //make cur = cur's left.
	return cur->val; //Once cur is the left most Node, return it's value.
}
...
{
	if (cur->left != 0) //While curren'ts left is not = 0
	{
		cur->left = _removeLeftMost(cur->left); //Recursively call the function on current's left.
		return cur;
	}

	//Otherwise return the right child.
	struct Node * newNode = cur->right; //Create a new node and store cur->right
	free(cur); //Free cur
	return newNode;
}
/*
...
{
	struct Node * tempNode; //Create a temporary Node
	if (compare(val, cur->val) == 0) //If the value is found in the BST
	{ 
		if (cur->right == 0) //If the right most element is empty
		{
			tempNode = cur->left; //Store the left in a temporary  node.
			free(cur); //Free cur
			return tempNode; //return temp node.
		}
		cur->val = _leftMost(cur->right); //Store the left most value in the current right
		cur->right = _removeLeftMost(cur->right); //Remove the left most node of the current right.
	}
	else if (compare(val, cur->val)== -1) // If its less 
		cur->left = _removeNode(cur->left, val); //reecursivly call remove node.
	else //If its more
		cur->right = _removeNode(cur->right, val); //Recursively call remove node
	return cur;//Else, return cur
