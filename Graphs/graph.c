...
{
	clearVisited(graph); // CLear the visited in graph

	if (source == destination) // If source is equal to the destination
		return 1;	//Return 1 (there is a path

	Deque *stack = dequeNew(); //Create a new deque called stack.
	dequePushBack(stack, source);  //Push source onto the stack.

	while (!dequeIsEmpty(stack))
	{
		Vertex *temp = dequeBack(stack); //Assign new value to temp.
		dequePopBack(stack); // Pop the front of hte stack.
		temp->isVisited = 1; // Change isVisited to 1;

		if (temp = destination) //If new temp is equal to destination then return 1
			return 1; 

		for (int i = 0; i < temp->numNeighbors; i++) //Do the same for all the neighbors.
		{
			if (temp->neighbors[i]->isVisited == 0)
				dequePushBack(stack, temp->neighbors[i]);
		}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	while (!dequeIsEmpty(stack))
	{
		Vertex *temp = dequeFront(stack); //Assign new value to temp.
		dequePopFront(stack); // Pop the front of hte stack.
		temp->isVisited = 1; // Change isVisited to 1;
